    ...
    <section className="relative z-0 mt-16 w-full rounded bg-white px-2 pb-4 md:pb-4">
      ...
      <div className="pt-11 md:max-w-[900px] md:pl-24 md:pt-7">
          ...
          <div className="grid grid-rows-2 items-center justify-center md:justify-start">
            {isEditing ? (
              ...
              <h1 className="flex items-end justify-center gap-2 text-xl md:justify-start md:text-2xl">
                <span className="text-sm">52期</span>{data.name}
              </h1>
            )}
        ...
        <div className="m-4 flex flex-col gap-2">
          {isEditing ? (
            <textarea col={3} name="profile" form="profileForm" className="rounded bg-slate-200 p-2" defaultValue={data.profile}></textarea>
          ) : (
              ...
              <div className="mb-2 w-full border-b pb-2">
                ...
                {isEditing ? (
                  <input
                    ...
                    name="learning_tags"
                    form="profileForm"
                    defaultValue={data.learning_tags.map((tag) => tag.name).join(",")}
                  ...
                  <dd className="ml-8 flex flex-wrap items-center justify-start gap-1">
                    {data.learning_tags?.map((tag) => (
                      <span key={tag.id} className="rounded-xl bg-slate-300 px-2">
                        {tag.name}
                      </span>
                    ))}
                  </dd>
              ...
              <div className="w-full">
                ...
                {isEditing ? (
                  <input
                    ...
                    name="learned_tags"
                    form="profileForm"
                    defaultValue={data.learned_tags.map((tag) => tag.name).join(",")}
                    className="w-full rounded bg-slate-200 p-2"
                    placeholder="Ruby, Ruby on Rails, JavaScript"
                  />
                ) : (
        ...
        {isEditing && (
          <form id="profileForm" className="flex w-full items-center justify-center" onSubmit={handleSave}>
            <button
              type="submit"
