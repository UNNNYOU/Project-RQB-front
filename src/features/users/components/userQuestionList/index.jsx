...
export default function UserQuestionList({ uuid }) {
  ...
  const allCount = useFetchData(`${Settings.API_URL}/users/${uuid}/all_questions_count?tab=${currentTab}`);
  const PER_PAGE = 10;
  ...
  return (
    <>
      <div>
        <ul className="grid grid-cols-3 gap-2 md:grid-cols-4">
          <li className="flex items-center justify-center">
            <button type="butotn" onClick={() => handleTabChange(Tabs.question)} className={`w-full rounded-t border-t-2 bg-white py-2 ${currentTab === Tabs.question ? " border-runteq-primary font-semibold text-runteq-primary" : "border-gray-400 bg-gray-400 text-white"}`}>質問</button>
          </li>
          <li className="flex items-center justify-center">
            <button type="butotn" onClick={() => handleTabChange(Tabs.answer)} className={`w-full rounded-t bg-white py-2 ${currentTab === Tabs.answer ? "border-t-2 border-runteq-primary font-semibold text-runteq-primary" : "border-gray-400 bg-gray-400 text-white"}`}>回答</button>
          </li>
        </ul>
      </div >
      ...
      <Pagination currentPage={Number(currentPage)} totalPage={TOTAL_PAGE} path={Routes.user(uuid)} />
